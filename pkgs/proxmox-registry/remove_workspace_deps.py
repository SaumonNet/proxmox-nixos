...
class WorkspaceCanonicalizer:
    def __init__(self, crate_path: Path, workspace_root: Optional[Path] = None):
        ...
        if not self.workspace_cargo_toml.exists():
            raise FileNotFoundError(f"Workspace Cargo.toml not found at {self.workspace_cargo_toml}")
        if not self.crate_cargo_toml.exists():
            raise FileNotFoundError(f"Crate Cargo.toml not found at {self.crate_cargo_toml}")
    ...
    def resolve_workspace_dependency(self, dep_name: str, dep_spec: Any, 
                                   workspace_deps: Dict[str, Any],
                                   workspace_members: Dict[str, Dict[str, Any]]) -> Any:
        ...
        if isinstance(dep_spec, dict) and dep_spec.get('workspace') is True:
            ...
            print(f"Debug: Resolving workspace dependency '{dep_name}'", file=sys.stderr)
            
            # First check if it's defined in workspace.dependencies
            if dep_name in workspace_deps:
                print(f"Debug: Found '{dep_name}' in workspace.dependencies", file=sys.stderr)
                ...
                print(f"Debug: Resolved '{dep_name}' to {resolved_dep}", file=sys.stderr)
                return resolved_dep
            ...
            # If not in workspace.dependencies, check if it's a workspace member
                ...
                print(f"Debug: Resolved member '{dep_name}' to {resolved_dep}", file=sys.stderr)
                return resolved_dep
            
            else:
                print(f"Debug: '{dep_name}' not found in workspace deps or members", file=sys.stderr)
                print(f"Debug: Available workspace deps: {list(workspace_deps.keys())}", file=sys.stderr)
                print(f"Debug: Available workspace members: {list(workspace_members.keys())}", file=sys.stderr)
                ...
                if not resolved_dep:
                    print(f"Warning: Could not resolve workspace dependency '{dep_name}' - no definition found in workspace", file=sys.stderr)
    ...
    def canonicalize_dependencies(self, deps: Dict[str, Any], 
                                workspace_deps: Dict[str, Any],
                                workspace_members: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for dep_name, dep_spec in deps.items():
            ...
            if resolved_dep and (
                isinstance(resolved_dep, str) or 
                (isinstance(resolved_dep, dict) and resolved_dep)
            ):
                canonicalized[dep_name] = resolved_dep
            else:
                print(f"Warning: Skipping empty dependency '{dep_name}' after resolution", file=sys.stderr)
    ...
    def canonicalize_manifest(self) -> Dict[str, Any]:
        ...
        if 'package' in canonicalized:
            ...
            try:
                with open(self.workspace_cargo_toml, 'r') as f:
                    workspace_data = toml.load(f)
                workspace_package = workspace_data.get('workspace', {}).get('package', {})
            except Exception:
                pass
            
            # Resolve workspace inheritance for package fields
            inheritable_fields = ['version', 'authors', 'edition', 'description', 
                                'documentation', 'readme', 'homepage', 'repository',
                                'license', 'license-file', 'keywords', 'categories',
                                'publish', 'metadata']
            
            for field in inheritable_fields:
                if isinstance(package.get(field), dict) and package[field].get('workspace') is True:
...
def main():
    ...
    parser.add_argument(
        "--in-place",
        action="store_true",
        help="Overwrite the original Cargo.toml file"
    )
