...
if [[ $# -lt 1 ]]; then
  echo "Please provide the name of the package to update."
  exit 1
fi

# Parse required arguments
PKG_NAME=$1
REPO_URL=https://git.proxmox.com/$PKG_NAME.git
OLD_VERSION=$(nix eval .#$PKG_NAME.version)
shift
...
while [[ $# -gt 0 ]]; do
  ...
  --version)
    shift
    VERSION=${1:?Error: --version requires a value}
    ;;
  --prefix)
    shift
    MESSAGE_PREFIX=${1:?Error: --prefix requires a value}
    ;;
  --root)
    shift
    SOURCE_ROOT=${1:?Error: --root requires a value}
    ;;
  --*)
...
if [ "$OLD_VERSION" = "\"$VERSION\"" ]; then
  echo "New version same as old version, nothing to do."
else
