...
async def start_updates(
    ...
    with contextlib.ExitStack() as stack:
        ...
        nixpkgs_root_process = await check_subprocess(
            "git", "rev-parse", "--show-toplevel", stdout=asyncio.subprocess.PIPE
        )
        nixpkgs_root = (
            (await nixpkgs_root_process.stdout.read()).decode("utf-8").strip()
        )
...
parser.add_argument(
    "--commit", "-c", dest="commit", action="store_true", help="Commit the changes"
)
parser.add_argument(
    "packages",
